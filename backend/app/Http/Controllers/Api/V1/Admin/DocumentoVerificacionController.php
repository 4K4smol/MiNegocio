...
class DocumentoVerificacionController extends ApiController
{
    ...
    public function preview(DocumentoVerificacion $documento): Response|JsonResponse
    {
        ...
        $mimeType = $disk->mimeType($documento->archivo) ?: $documento->mime_type ?: 'application/octet-stream';

        if (! $this->esPrevisualizable($mimeType)) {
            return $this->error('Este tipo de documento no se puede previsualizar.', 415);
        }

        $this->service->registrarVisualizacionDocumento(request()->user(), $documento->load('solicitudVerificacion'));

        $filename = str_replace(['"', '\\'], '', $documento->nombre_original ?: basename($documento->archivo));
