...
class UpdateServicioRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $id = (int) ($this->route('servicio') ?? $this->route('id'));

        return [
            'empresa_id' => ['sometimes', 'nullable', 'integer', 'exists:empresas,id'],
            'tipo_negocio' => ['nullable', 'string', 'max:50'],
            'codigo' => ['nullable', 'string', 'max:50', Rule::unique('servicios', 'codigo')->where(fn ($query) => $query->where('empresa_id', $empresaId))->ignore($id)],
            'nombre' => ['sometimes', 'required', 'string', 'max:150'],
            'descripcion' => ['nullable', 'string'],
            'unidad_servicio' => ['sometimes', 'required', 'string', 'max:30'],
